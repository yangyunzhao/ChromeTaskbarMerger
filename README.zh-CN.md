...
V3 定位为易用性、视觉一致性和缺陷修复版本，原则上不增加新的产品能力。目前重点包括：

- 让受管最大化/还原按钮的图标与真实状态和操作语义一致；
- 统一内置标签栏在普通、最大化、Snap、F11、DPI 和工作区变化后的目标位置；
- 最大化时让标签栏在视觉上融入 Chrome，获得类似 WindowTabs 的统一窗口观感，但不复制或
  注入 WindowTabs；
- 完成恢复、闪烁、焦点、资源占用和既有功能回归收口。

开发计划分为六个带验收门禁的 Phase，从视觉规范和诊断基线一直到 3.0.0 发布候选。最大化视觉
方案会在实现前根据用户提供的参考图片确认。V2 的恢复和窗口可达性仍是硬门槛；不支持的视觉
方案必须安全回退到已经验证的 2.0.0 布局。

完整计划见 [V3 易用性、视觉完善与缺陷修复计划](docs/V3_REQUIREMENTS.md)。会扩大产品能力的需求
统一进入独立的 [V4 功能需求池](docs/V4_BACKLOG.md)，不扩大 V3 范围。
