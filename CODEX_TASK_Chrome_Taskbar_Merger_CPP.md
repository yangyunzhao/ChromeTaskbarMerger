...
#### Phase 2 实施进度（2026-07-15）

已实现 `--experiment` 交互入口，以及方法 A、方法 B 的独立移除与恢复路径。实验会在操作前重新核对 HWND、PID、TID 和窗口类；只有用户输入精确的 `APPLY` 才会修改所选窗口。方法 B 保存完整原始 `GWL_EXSTYLE`，恢复时写回并核对完整原值。已应用的修改由作用域恢复保护负责兜底；恢复前还会重新核对窗口身份，避免陈旧或复用 HWND 影响其他窗口。

已完成的自动验收：

- Debug 与 Release 构建均成功，`/W4` 下无警告；
- Debug 与 Release CTest 均为 1/1 通过；
- `ITaskbarList` 的 COM 初始化与 `HrInit` 在本机成功；
- 使用本测试进程创建的两个不可见窗口验证了方法 B 的完整往返：只修改所选窗口、保留无关样式位、精确恢复原样式，重复恢复安全且幂等；
- 无效 HWND 和未修改状态的恢复路径通过；
- 选择窗口前取消，以及方法 A、方法 B 在输入 `APPLY` 前取消，均以退出码 0 安全结束且没有修改真实窗口；
- 实验结果会记录 HRESULT、Win32 错误码、修改前后扩展样式、恢复结果，以及任务栏、Chrome 可用性、WindowTabs 和 Alt+Tab 的人工观察。

人工验收环境：Windows 11 Pro 10.0.26200、Chrome 150.0.7871.115、WindowTabs `ss_2026.07.14`。

- 方法 A：`DeleteTab` 返回 `S_OK` 后目标任务栏按钮真实消失；Chrome 窗口保持打开、可到达和可操作，WindowTabs 仍可正常切换；目标窗口暂时不出现在 Alt+Tab 中。`AddTab` 返回 `S_OK` 后按钮无需重启 Explorer 即恢复。视觉结论为 `PASS`。
- 方法 B：扩展样式从 `0x00200100` 成功改为 `0x00200180`，按钮真实消失；但目标 Chrome 无法通过 WindowTabs、Alt+Tab 或本次测试采用的正常方式到达，因此不满足 V1 的最低可用性要求。恢复时扩展样式精确回到 `0x00200100`，按钮也恢复。视觉结论为 `FAIL`，该路径不作为正式方案。

当前状态：`PASS`。Alt+Tab 缺项属于已记录且在 V1 中允许的兼容性代价，不阻塞方法 A，因为 Chrome 本身仍可由 WindowTabs 到达和操作。Phase 3 应只使用方法 A 作为默认任务栏控制机制；方法 B 仅保留在实验代码中用于诊断，不应作为回退自动应用。详细证据见 `tests/manual_test_plan.md`。

### Phase 3：固定主入口 MVP

目标：把已验证的方法 A 扩展为“多个 Chrome 窗口、任务栏固定只保留一个入口”的最小常驻版本。

实现内容：

- 启动时若前台窗口是可管理 Chrome，则选它为固定主入口；否则按稳定、可解释的顺序选择一个；
- 主入口有效期间，不因 WindowTabs、Alt+Tab 或鼠标切换前台窗口而更换入口；
- 使用 `ITaskbarList::DeleteTab` 移除其他可管理 Chrome 窗口的任务栏按钮，并使用 `AddTab` 恢复；不得自动回退到方法 B；
...
- 测试只恢复本程序记录为已修改的窗口；
- 测试只有 `DeleteTab` 成功的窗口才进入已修改状态，恢复时每个窗口只调用一次 `AddTab`，重复恢复保持幂等；
...
#### Phase 3 实施进度（2026-07-15）

已实现显式 `--manage` 控制台会话。用户输入精确的 `MANAGE` 后，程序才会初始化方法 A 并应用固定入口规则。会话提供：

- `s`：立即重新扫描；管理启用时同步固定入口，暂停时只读扫描；
- `a`：应用或恢复管理并立即扫描；
- `r`：只恢复本会话成功移除的任务栏按钮并暂停；
- `q`：恢复全部后正常退出；
- `h`：显示帮助。

主入口使用 HWND、PID、TID 和窗口类共同标识。首次同步优先采用启动时的前台可管理 Chrome；否则按当前窗口清单中的最低 HWND 选择稳定入口。主入口身份仍有效时，后续前台变化不会替换它。只有 `DeleteTab` 成功且产生恢复义务的窗口才进入状态表；重复同步不会重复调用 `DeleteTab`，重复恢复不会重复调用 `AddTab`。恢复协调失败时，本轮不会继续移除新窗口。

鉴于 Phase 2 已确认被移除窗口不会出现在 Alt+Tab 中，Phase 3 在开始管理和每次同步前检查 `WindowTabs.exe`。若它不可用，程序拒绝新增移除，并对本会话状态执行恢复后暂停。Phase 3 没有后台轮询；WindowTabs 在命令等待期间退出时，需输入 `s` 或 `r` 触发检查与恢复，自动监控留到 Phase 4。

已完成的自动验收：

- Debug 与 Release 构建均成功，`/W4` 下无警告；
- Debug 与 Release CTest 均为 2/2 通过；
- 伪窗口清单覆盖 0、1、3、5 个窗口；
- 已验证前台 Chrome 选择、非 Chrome 前台的稳定回退、固定主入口不随前台变化、恢复后重新应用，以及零窗口清理；
- 已验证重复同步/恢复幂等、只恢复成功移除项、主入口关闭后提升、恢复失败保留重试、恢复协调失败时停止新增移除；
- 已验证同一 HWND 的 PID 身份变化时先解析旧状态，再把新身份作为独立窗口处理；真实 Win32 控制器还会在恢复前再次核对 HWND、PID、TID 和类名；
- `--manage` 在输入 `MANAGE` 前取消返回 0，未初始化 COM 且未修改真实任务栏；
- `--help`、无参数启动、`--list`、Phase 2 取消路径和非法参数退出码回归通过。

人工验收环境沿用 Phase 2：Windows 11 Pro 10.0.26200、Chrome 150.0.7871.115、WindowTabs `ss_2026.07.14`。

- 1 个窗口：同步和重复扫描均保持 0 个跟踪移除；正常退出恢复成功；用户确认始终保留 1 个入口。
- 3 个窗口：首次恰好成功调用 2 次 `DeleteTab`；重复扫描保持同一主入口 HWND，报告 `Already removed=2` 且没有新任务栏调用；`r` 成功调用 2 次 `AddTab`，`a` 再成功调用 2 次 `DeleteTab`，`q` 再成功调用 2 次 `AddTab`。用户确认管理时只剩 1 个入口、全部窗口仍可用，恢复时 3 个入口重新出现。
- 5 个窗口：首次恰好成功调用 4 次 `DeleteTab`；重复扫描保持同一主入口，报告 `Already removed=4` 且没有新调用；`q` 恰好成功调用 4 次 `AddTab`。用户确认管理时只剩 1 个入口、全部窗口仍可用，退出后 5 个入口恢复。
- 三轮始终使用固定主入口 `HWND=0x0000000000020332`。日志合计记录 7 次成功同步、8 次成功 `DeleteTab`、8 次成功 `AddTab` 和 4 次成功恢复流程；HRESULT 与 Win32 错误码均为 0，未出现 `FAIL`、警告或恢复遗留。

当前状态：`PASS`。PowerShell transcript 对三、五窗口的原生输出记录不完整，因此 API 证据以程序自身 UTF-8 日志为准；用户的实际任务栏观察提供不可自动化的视觉证据。Phase 4 允许开始。详细记录见 `tests/manual_test_plan.md`。
...
#### Phase 4 实施进度（2026-07-15）

已完成实现：

- 新增基于 `std::chrono::steady_clock` 的 `ScanSchedule`，默认每 2 秒扫描一次；扫描完成后重新计算下一截止时间，手动扫描会推迟下一次周期扫描，避免紧邻重复执行；
- 控制台循环使用 `WaitForSingleObject` 同时等待键盘输入或下一次扫描截止时间，暂停状态使用无限期阻塞等待，不创建后台线程且不存在忙循环；
- 自动扫描会纳入新 Chrome 窗口、清理已关闭的非主窗口状态、在主入口关闭后提升新主入口，并在全部 Chrome 关闭后清空状态、重开后重新建立单入口；
- `s`、`a`、`r`、`h`、`q` 改为单键命令，无需按 Enter；`r` 恢复并暂停，`a` 立即重新扫描并恢复管理；所有任务栏修改仍在同一主线程串行执行；
- 自动扫描时若发现 `WindowTabs.exe` 不可用，会停止新增移除、恢复本会话已跟踪的任务栏按钮并暂停管理；
- 无变化的周期扫描保持安静，不重复调用 `DeleteTab` / `AddTab`，仅在主入口变化、发生任务栏操作或失败时输出并记录报告。

已完成自动验收：

- 策略测试覆盖运行中新建窗口、关闭非主窗口、关闭主入口、全部关闭后以新身份重开、暂停/恢复、重复扫描和 HWND 身份复用；
- 可控时钟测试覆盖精确 2 秒截止时间、即时请求合并、手动扫描去抖和非法间隔保护，不依赖真实等待；
- Debug 与 Release 均在 `/W4` 下无警告构建，CTest 均为 3/3 通过；
- `--help`、无参数启动、`--list`、`MANAGE` 前安全取消和非法参数退出码回归通过；当前环境不满足 Phase 2 实验窗口前置条件时，`--experiment` 在任何修改前按设计返回 4。

人工验收于 2026-07-15 在真实 Chrome、WindowTabs 和 Windows 任务栏上完成：

- 从 1 个 Chrome 窗口开始管理，依次新建第 2、3 个窗口后，用户均确认在等待 3 秒内仍只有 1 个任务栏入口；
- 关闭非主窗口后固定入口保持不变；关闭固定主入口后，剩余窗口自动恢复并成为新主入口，任务栏始终保持 1 个入口；
- 全部关闭后状态收敛到 0 个窗口和 0 个跟踪项；重新打开 3 个窗口后再次自动收敛到 1 个入口；
- `r` 成功恢复全部按钮并暂停，等待超过一个扫描周期后没有重新隐藏；`a` 成功恢复管理并重新收敛为 1 个入口；
- 空闲运行至少 15 秒时 CPU 表现正常，无持续异常占用；`q` 后全部按钮恢复，用户未观察到异常；
- 本次 17:36～17:42 日志记录 11 次成功生命周期同步、6 次实际 `DeleteTab` 成功、5 次实际 `AddTab` 成功，以及 1 次因原窗口已关闭而安全跳过的恢复协调；所有实际 API 的 HRESULT 和 Win32 错误码均为 0；
...
### Phase 7：随 Windows 登录自动启动

目标：保持免安装便携形态，让用户可以从托盘或 INI 配置当前用户登录 Windows 后自动运行，并可靠处理 WindowTabs 启动顺序不确定的问题。

实现内容：

- `ChromeTaskbarMerger.ini` 新增 `start_with_windows=false`，默认关闭；
- 托盘新增带勾选状态的“随 Windows 登录自动启动”；
- INI 作为期望状态，`HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 作为实际启动入口；
- 托盘切换先原子保存 INI，再更新注册表，注册失败时回滚配置；
- 注册命令使用带引号的完整便携 EXE 路径和内部 `--autostart` 标记；
- 手工编辑配置只在完全退出并重新运行后生效，不实时监控文件；
- 自动登录启动时，如果 WindowTabs 未就绪，使用非阻塞定时器每 3 秒重试，最长 120 秒；
- 等待期间不移除任何 Chrome 任务栏按钮；WindowTabs 就绪后自动开始管理，超时后保持暂停；
- 便携目录移动后，手动从新位置运行一次会修复注册路径；
- Windows 设置或任务管理器禁用启动应用时，以系统级选择为准，不操作未公开的 `StartupApproved` 数据。

Codex 自动验收：

- 测试布尔配置默认值、大小写、无效值安全回退；
- 测试托盘配置保存保留注释、扫描间隔和其他节，并清理重复键；
- 在测试专用临时注册表子键验证创建、查询、幂等、路径修复和删除，测试后不遗留注册项；
- 测试带空格和中文路径的命令引号，以及 Windows `Run` 命令长度限制；
- 使用可控时钟测试 3 秒重试、120 秒超时和停止状态，不实际等待；
- Debug/Release 全新构建、全部 CTest、便携安装和 ZIP 内容验证通过；
- 验证 Release 为 x64 Windows GUI，且不动态依赖 MSVC 运行库。

人工验收：

1. 从托盘启用自动启动，确认菜单勾选且 INI 变为 `true`；
2. 退出并重新登录 Windows，确认托盘程序自动出现；
3. 分别测试 WindowTabs 先启动，以及 ChromeTaskbarMerger 先启动、WindowTabs 延迟约 15 秒的情况；
4. 两种顺序最终都应进入“管理中”，Chrome 任务栏仍只保留一个入口；
5. 从托盘关闭自动启动，再次登录后确认程序不再自动出现；
6. 直接修改 INI 后完全退出并重启程序，确认启动注册状态按配置收敛。

通过标准：配置文件、托盘勾选和 Windows 启动注册保持一致；启动顺序不造成永久误暂停；关闭功能后不再自动运行；原有任务栏管理和恢复能力全部保持通过。
...
`SetWinEventHook` 属于 Phase 8 的优先增强；如果实测低频扫描无法满足新建、关闭窗口的体验，或资源占用不理想，可以提前引入，但必须保持任务栏修改仍由主线程串行执行。
...
不得在非 Chrome 程序激活时把所有 Chrome 都从任务栏移除。让任务栏入口动态跟随当前活动 Chrome 属于 Phase 8 可选功能。
...
### `win_event_monitor.*`（Phase 8 可选，或 Phase 4 实测需要时提前引入）
...
- Phase 8 可以在确有收益时改为事件驱动或事件加低频扫描；
