...
- Phase 3 尚未实现 Phase 4 的持久化窗口布局恢复；异常结束后任务栏恢复但窗口仍重叠时，
  请手动移动窗口，不要结束 Chrome 进程。
...
图标而不是原生“还原”图标，不影响再次点击执行还原；用户要求在最后一个 Phase 尝试解决，
已加入 Phase 7 发布前专项与安全门槛。用户在理解 Windows Snap 是窗口贴靠/分屏后，补充
确认左/右 Snap 也已通过。
...
`PASS`。受管最大化按钮图标差异不影响功能，保留为 Phase 7 发布前专项，不阻塞 Phase 3。
